...
class SendDailyMealReminder extends Command
{
    ...
    public function handle()
    {
        ...
        User::query()->chunkById(500, function ($users) use ($notificationService, $engagement, $now, &$due, &$skipped) {
            // Costs nothing: whose clock reads 10:00 is arithmetic on a column
            // that is already loaded.
            $candidates = $users->filter(
                fn (User $user) => ReminderWindow::isDue($user->localNow($now), self::TARGET_HOUR, self::TARGET_MINUTE)
            );
